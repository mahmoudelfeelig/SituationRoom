...
function EventDelta({ event }) {
  return (
    <dl className="os-playback-delta">
      ...
      <div>
        <dt>Authority</dt>
        <dd>{event.decision.changed ? "Canonical change" : event.view.changed ? "View only" : "Unchanged"}</dd>
      </div>
    </dl>
  );
}
...
export function DecisionPlayback({ receipts, activeCaseId }) {
  ...
  if (!playback.events.length) {
    return <p className="os-empty-state">No workspace receipts exist for this case yet.</p>;
  }
  ...
  return (
    <section className="os-decision-playback" aria-labelledby="decision-playback-heading">
      <header className="os-playback-heading">
        <div>
          <span className="os-eyebrow">Decision time machine</span>
          <h3 id="decision-playback-heading">Trace what changed and why</h3>
          <p>Canonical decisions, presentation-only motion, imports, human authority, and browser-agent work stay visibly distinct.</p>
        </div>
        <div className="os-playback-summary" aria-label="Playback summary">
          <span><strong>{playback.summary.total}</strong> events</span>
          <span><strong>{playback.summary.canonicalChanges}</strong> canonical</span>
          <span><strong>{playback.summary.presentationChanges}</strong> view only</span>
      ...
      {compareOpen ? (
        <section className="os-playback-compare" aria-labelledby="playback-compare-heading">
          <div className="os-playback-compare-controls">
            <h4 id="playback-compare-heading">Compare two receipts</h4>
          ...
          {comparison ? (
            <div className={`os-playback-comparison-result ${comparison.decisionChanged ? "is-canonical" : "is-view-only"}`} role="status">
              <strong>{comparison.decisionChanged ? "Decision changed" : comparison.viewChanged ? "Presentation changed" : "No authoritative delta"}</strong>
              <p>{comparison.summary}</p>
              {comparison.changedEntityIds.length ? <span>{comparison.changedEntityIds.length} causal entities: {comparison.changedEntityIds.join(" · ")}</span> : null}
            </div>
          ) : null}
        </section>
      ) : null}
      ...
      <div className="os-playback-workbench">
        ...
        {selected ? (
          <article className={`os-playback-inspector scope-${selected.scope}`} aria-live="polite">
            <header>
              <span className="os-playback-seal"><IconPlayerPlay size={18} /> {selected.scope}</span>
            ...
            <div className="os-playback-causal-path">
              <span className="os-eyebrow"><IconRoute size={16} /> Causal entities</span>
              ...
              ) : <p>No canonical entity was changed by this receipt.</p>}
            </div>
