...
function packetHtml(packet) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${html(packet.case.title)} · decision packet</title><style>
  ...
  <header><small>SITUATIONROOM · CITED DECISION PACKET</small><h1>${html(packet.case.title)}</h1><p>${html(packet.contract.question)}</p></header>
...
function xlsxBytes(rows) {
  return zipSync({
    ...
    "xl/workbook.xml": strToU8(`<?xml version="1.0" encoding="UTF-8"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="Decision packet" sheetId="1" r:id="rId1"/></sheets></workbook>`),
