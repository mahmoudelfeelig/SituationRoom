...
export function stableStringify(value) {
  ...
  function visit(candidate, path) {
    ...
    if (typeof candidate === "bigint") return JSON.stringify(candidate.toString());
    if (candidate instanceof Date) return JSON.stringify(candidate.toISOString());
    if (candidate instanceof Uint8Array) {
      return `[${Array.from(candidate, (byte) => String(byte)).join(",")}]`;
    }
    if (Array.isArray(candidate)) {
      if (seen.has(candidate)) throw new TypeError(`Circular value at ${path}.`);
      seen.add(candidate);
      const result = `[${candidate.map((entry, index) => visit(entry, `${path}[${index}]`)).join(",")}]`;
    ...
    throw new TypeError(`Unsupported ${typeof candidate} value at ${path}.`);
  }
