...
- Production deploys must consume the exact checksummed image produced by a successful unsuperseded `main` CI run. The repository delegates through the immutable shared release gateway using OIDC and must not contain or inherit host credentials. Keep release serialization, public smoke checks, and automatic rollback in the centralized controller; routine application deploys must not mutate Cloudflare or the shared Caddy route.
